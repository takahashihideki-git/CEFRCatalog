...
- **第1層：カタログ** ── 四類型の範型作成済み。**周回方式**で進行中：**第1周（5検証）完了** ── 四類型すべてが内部分化を持つと確定（表出型三分割／主張型二相／指示型は苦情・依頼別立て／儀礼型二分）、類型横断軸3候補（往復・応答／書面フォーマル一段／縦横分業）。**第2周（横断判定）進行中**：判定1（授受↔Q&A統合＝一行為二相、25→24行為）完了。次は判定2（好悪の表明／理解状態の表明の帰属）
...
data/          参照用の一次データ（JSON）
  ...
  inventory_170to24.json         採用170記述文→24発語内行為の写像
  act_to_satype.json             24行為→サール4類型
  act_phases.json                一行為二相の正準記録（事実情報の授受＝質問応答相15＋情報管理相15）
...
- `inventory_170to24.json`: ユニーク行為数 24
- `act_phases.json`: 事実情報の授受30件を質問応答相15＋情報管理相15に完全分割（重複なし）
