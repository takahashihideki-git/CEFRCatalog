...
- **第1層：カタログ** ── 四類型の範型作成済み。**周回方式**で進行中：**第1周（5検証）完了** ── 四類型すべてが内部分化を持つと確定（表出型三分割／主張型二相／指示型は苦情・依頼別立て／儀礼型二分）、類型横断軸3候補（往復・応答／書面フォーマル一段／縦横分業）。**第2周（横断判定）進行中**：判定1〜4完了（授受↔Q&A統合／帰属判定／横断軸昇格／類型再編＝**下位系12系が第一分類層・サール4類型は出自タグ・梯子型4分類を全行為付与**）、25→**23行為**。残るは篩対象外62件の精査（第2周-5）
