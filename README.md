...
- **第1層：カタログ** ── 四類型の範型作成済み。**周回方式**で進行中：**第1周（5検証）完了** ── 四類型すべてが内部分化を持つと確定（表出型三分割／主張型二相／指示型は苦情・依頼別立て／儀礼型二分）、類型横断軸3候補（往復・応答／書面フォーマル一段／縦横分業）。**第2周（横断判定）進行中**：判定1（授受↔Q&A統合＝一行為二相）・判定2（好悪＝意見表明系帰属／理解状態→明確化要求と統合）・判定3（横断軸昇格：往復・応答＝確定／書面フォーマル一段＝条件付き／縦横分業＝説明原理「梯子は管理課題に宿る」＋梯子型三分類）完了、25→**23行為**。次は類型再編（下位系・梯子型の全行為付与・cross_axes.json新設）
...
data/          参照用の一次データ（JSON）
  ...
  inventory_170to23.json         採用170記述文→23発語内行為の写像
...
- `inventory_170to23.json`: ユニーク行為数 23
