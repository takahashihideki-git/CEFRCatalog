...
def main():
    ...
    for f in files:
        ...
        # 「在庫」の比喩用法（表現・語彙の持ち合わせ）は退役 ── 商品在庫の実義（取引146/152・問い合わせの照会）は対象外のため、裁定済み語形のみ検出（カタログ22 Hideki精読。「の在庫は」「の在庫も」は実義と衝突するため意図的に拾わない ── 網羅は精読）
        for w in ('在庫し', '在庫する', '在庫化', '受け在庫', '在庫が薄', '在庫が要る', '在庫を持', '在庫で回', '在庫として', 'の在庫が', 'の在庫を', 'の在庫と', 'の在庫で'):
            if w in t:
                errors.append(f"{name}: 用字「在庫」の比喩用法「{w}」（語彙・表現パターン・手持ち等の平明語へ、カタログ22裁定。商品在庫の実義は可）")
        if '系列' in t:
        ...
        for pat, lab in (
            ...
            (r'まだ読んでいない章|ここまでの章|ここまでの行為|いま読み終えた', '読書状態の仮定（→「この先の章」「行為の章を」等の文書側の記述へ）'),
