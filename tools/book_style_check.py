...
def main():
    ...
    for f in files:
        ...
        # 読者の読書行為の言い切り（規律3追補、カタログ17検品。「思い出してほしい」等の軽い誘いは精読判定）
        for w in ('章で読んで', '読んだ読者'):
            if w in t:
                errors.append(f"{name}: 読者行為の言い切り「{w}」（内容主語の軽い形へ：「で見た」「に登場する」）")

        # 5.7 章の外への読了前提（規律3の方向例外撤廃、判断(at)カタログ18。読了前提は自章内のみ）
        #     自章内の合法形（「〜の段で見た」「No.Nで見た」）は「の章」「巻頭」「幕間」を先行語に持たないため発火しない。
        #     文境界（。・改行）をまたぐ照合はしない。
        for pat, lab in (
            (r'の章[^。\n]{0,30}?[でに](?:も)?見た', '章への読了前提（位置中立の現在形へ：「〜の章にも現れる」「〜の章が受け持つ」）'),
            (r'巻頭[^。\n]{0,10}?で(?:も)?見た', '巻頭への読了前提（文書主語へ：「巻頭に掲げた」）'),
            (r'幕間[^。\n]{0,10}?[でに](?:も)?見た', '幕間への読了前提（文書主語へ：「前の幕間が描く」）'),
            (r'(?:の章|幕間|巻頭)[^。\n]{0,15}?を?思い出し', '章の外への記憶訴求（規律3：読了前提は自章内のみ）'),
            (r'まだ読んでいない章', '読書状態の仮定（→「この先の章」等の位置の記述へ）'),
        ):
