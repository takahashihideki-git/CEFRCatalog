...
**判断を1つ追記したら、二次コピー発生箇所を定型パトロールする（散文追随チェックリスト）**。機械ゲートは正準↔正準しか見ない ── 判断が増えるたびに、その事実の二次コピーが散文側に3〜6か所生まれ、パッチが新しい正準と直近の散文だけを触ると古い場所のコピーが取り残される（実例：カタログ12検品で6件＋パトロール1件＝計7件の残置を一括補修）。パッチを閉じる前に次を必ず見る：①README「いまどこにいるか」 ②READMEディレクトリ構成 ③README復元検証値（新assertの行追加と、既存行の数値・文言） ④引き継ぎ書§1 ⑤引き継ぎ書§2（当該判断の本文） ⑥引き継ぎ書§7（同じ事項の申し送りが残っていれば取り消し線＋完了へ） ⑦企画書（構造の数値・見出しに触れた場合） ⑧CLAUDE.md自身。数値（件数・枚数・対数）を変えた判断は、その数値の**全文書grep**を追随の一部とする。