...
```bash
python3 restore.py            # 1224件／篩265・ADOPT182／行為22／二相31+17+31／分類22・下位系12／範型4照合／検証範型5照合／区分分割7／全数シート意見31照合
...
- **第3周の全数シート（`catalog_*.json`）は生成物**。ソースは `prototypes/<act>_full_prototype.py`（en/jp は data から構築時取得＝単一情報源）。突き合わせは「`.py` を実行して再生成 → `git diff` が空」で行う。あわせて `tools/exponent_level_check.py` を通す（第3周-0の品質ゲート）
