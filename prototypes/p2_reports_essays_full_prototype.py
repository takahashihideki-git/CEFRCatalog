...
351: (
 ["This essay makes three claims: that the policy is affordable, that it is popular, and — most importantly — that it is already working elsewhere.",
  "Two objections remain; I take the weaker first.",
  "To return to the three claims in order: the policy costs less than the current scheme, most residents support it, and two neighbouring towns already run it."],
 "読み手が重要な点を見つけやすい、適切で効果的な論理構造を与える。",
 "一貫性・結束性の書面上端＝構成の設計。予告と回収の対応、段落機能の一貫。構成の設計そのものが課題の行で、型の標本は四つ：主張を冒頭段落の末尾に置く／各段落の第一文を論点文にする／転換点に予告を置く（Two objections remain; I take the weaker first.）／結論で論点を同じ順序で回収する（To return to the three claims in order: ...）。例文は冒頭の予告・転換点の予告・結論の回収の三本（カタログ25で型の標本の散文を例文欄からhowwellへ移し、標本内の英文を例文行へ起こした）。",
