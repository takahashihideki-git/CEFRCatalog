...
(549, '口頭', ["This one, please. （指差して）", "Two coffees, please."],
 ...
 "（起点）取引は最も早い級Pre-A1で始まる（Pre-A1起点は挨拶・授受・自己情報と本行為の4行為）── 身振りが正規の支えとして明記される。"),
...
DISCUSSION = [
"【管理課題の梯子 ── 取引の複雑化、Pre-A1からC1へ】 最も早い級Pre-A1の起点（549。Pre-A1起点は挨拶・授受・自己情報・取引の4行為で、指示型では本行為のみ）から、C1の役割反転（691）までを持つ最長帯の管理梯子。梯子の実体は、指差し購買（549）→注文・購買の定型（543/542）→窓口取引（540）→複合場面（539）→切実な領域（545）→用件の完遂（537）→問題対応（703）→詳細の明確化（700）→条件交渉（695）→サービス役（691）という取引そのものの複雑化であり、cross_axesの管理課題と一致する。支えの脱落（549の身振り→543）と場・内容の負荷増が交互にレベルを刻む。",
