...
DISCUSSION = [
 ...
 # ¶5 横串
 "効く how well 軸の主軸は、他の二族と違って一貫性・結束性ではなく**叙述の正確さ**である（266 reliably・268 precision・263 clearly が直接それを言う）。次いで話題の展開（工程・一連の手順の網羅）、語彙の範囲（職務・学術の専門語彙が265／264で必須になる）。社会言語的適切さは第1柱ほど前景化しないが、上段で反転した形で効く ── 婉曲さが適切さでなく不正確さとして罰される。本スケールではB2+〜C1の全行（265・263・264）がこの向きを持つ（全数確認）。同じ向きは22行為の上級にも現れる ── 例：意見492（with precision、罰し方まで不正確さ）、苦情526（B1で身につけた緩衝がB2+で危険物になる）。22行為側の件数は規準で変わる（狭＝不正確さとして罰される：492の1件／広＝緩衝が上級で逆に働く：6件以上）ため数えない。旧文面「22行為のどこにも無かった配置」（カタログ10）と、補遺2の「B2+の単発で三度」「帯の広さが本スケールの配置」は、いずれも規準なしの判定だったため撤回（カタログ26補遺2・3）。流暢さは全行で明示されない（口頭第一号の241 fluently のような行を持たない）── 情報提示は言い直しても情報が届けば達成される営みだからで、口頭スケールでありながら時間拘束が課題に入らない。質フラグ該当行は**0件**：271は順序接続語を名指しするが「XからYへ道順を教える」という課題を持つ本体行であり、課題を持たない技法行（333「Can clearly signal chronological sequence」）とは別種である（書面366と同処理、判断(ae)）── これで口頭3スケールすべて0件、「質フラグは全て書面側」の規則性候補は反証テストを生き延びた。最後に、本スケールは**C1で打ち止めでC2行を持たない** ── 上端が横串に溶ける現象（234／325／351）が起きない初のスケールであり、溶解が起きるのは帯がC2まで届くスケールに限られるという条件が、ここで裏側から確認される。",
]
