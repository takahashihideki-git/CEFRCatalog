...
SCOPE = '全数（1件。書面1）'
TYPE_ = '主張的（assertive）── 起きたこと・したことを語る。下位系＝叙述系、梯子型＝外部化型（外部化先＝第2柱・産出-談話構築の語り糸（正準：data/p2_threads.json）。設計判断(i)・(aa)。第3周-5第三陣で確定）'
ESSENCE = '起きたこと・したことを相手に語る。やり取りの帳簿にはオンライン投稿の1件のみが立ち、叙述の本体は第2柱・産出-談話構築の語り糸（口頭10行＋書面9行）にある ── この行為はその窓口。'
...
(680, '書面', ["Went hiking with friends today — beautiful weather, great views!", "Tried a new ramen place. Loved the noodles."],
 ...
 "やり取り帳簿における叙述の唯一の行 ── 叙述の実体は第2柱の語り糸（正準：data/p2_threads.json。680は書面投稿のため近接はCreative writing側の語り糸、口頭側=describing experienceへは並行対で接続）で展開し、本行為はやり取りへの接続点。"),
]

ORDER = [680]

DISCUSSION = [
"【外部化型の純粋形 ── 帳簿一行の窓口行為】 やり取り側の記述文は680のみで、消失点は起点と同時（A2+）。叙述の梯子（過去の語り・経験の構造化・物語）はまるごと第2柱・産出-談話構築の語り糸（describing experience＋Creative writing、正準：data/p2_threads.json）へ外部化されており、本行為は「やり取りの中で叙述が顔を出す最小の窓」── 設計判断(i)（産出との境界）の帳簿上の証拠。",
...
"【点検総括・語彙裁定】 外部化型で確定（仮判定どおり）。第2柱の範型2枚の完成を受け、窓口の参照は語り糸へ張り替え済み（判断(aa)・CEFRカタログ8）── 680（書面投稿）の近接はCreative writingの語り糸、口頭はその並行対であり、糸がモードに先立つ構造がそのまま参照に写る。語彙裁定（判断(s)）：ramen・noodles（680・A2+行）＝食物対象スロット（(s)-1）として保持（hiking は収録内で通過）。",
]
