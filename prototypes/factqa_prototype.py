...
ROWS = [
 ...
 ("書面","A2+",642,
  ...
  "自己完結の平叙記述から、書面の「往復する情報交換」へ。書面系列は No.649（A1最小形）→642→640→633 と授受・情報管理相に連なる。一方向の自己紹介は別行為「自己に関する情報提供」（No.643等）を参照。"),
]
...
【口頭は往復、書面は自己完結】
情報交換はCEFRでは圧倒的に口頭（Information exchange スケール）に記述が集中し、書面はCorrespondence由来の情報記述が少数あるのみ。この非対称は本質的で、事実の授受は本来リアルタイムの往復（Q — A）に最も自然に宿る。書面に移ると、口頭の省略（主語や述語の脱落）が使えず、自己完結した平叙文を連ねる必要が生じる ── 話し言葉の省略を書面に持ち込むと非文になる、という書面固有の壁が現れる。（第2周-1の統合判定〔授受↔Q&A＝一行為二相〕により、書面系列 No.649/642/640/633 は本行為の正当な行となった。一方向の自己紹介メール（No.643）は「自己に関する情報提供」の領分であり、本範型から除いて相互参照に改めた。）
