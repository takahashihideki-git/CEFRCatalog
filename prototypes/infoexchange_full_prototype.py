...
DELTA_EDITS = {
 ...
 555: "情報の授受に「照合」という品質管理の層が乗る。B1+の本質（with some confidence）は管理の自動化。363（報告と小論・B1+）と同文級の柱間再掲 ── 蓄積した事実情報の交換・照合・確認が、やり取り（授受）と書面産出（報告）の両帳簿に立つ（相互参照、判断(ai)）。",
 554: "口頭系列の到達点。梯子の頂点にあるのは高度な文法ではなく、伝達の信頼性という運用の質である。266（情報のまとまった提供・B2）と同文級の柱間再掲 ── 詳細情報の確実な伝達（reliably）が、やり取りと独話産出の両帳簿に立つ（相互参照、判断(ai)）。",
}
